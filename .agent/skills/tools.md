...
This repo vendors three tools under `tools/` (see [tools/README.md](../../tools/README.md)). Use them for any log analysis, simulation, or NT4 task before writing custom code.
...
| log-sync | [tools/log-sync/](../../tools/log-sync/) | Pulls match logs off the roboRIO to Google Drive between matches |
...
Example:
```
/scope analyze <path-to>.wpilog and tell me if the drivetrain was tracking its setpoints
```
...
> Using wpilib-agent-tools, analyze <path-to>.wpilog and tell me which subsystems are not meeting commanded setpoints.

> Using wpilib-agent-tools, create a sandbox and diagnose why the scoring sequence sometimes fires before alignment.
...
| Task | Tool |
|---|---|
| "Why was this mechanism oscillating in that match?" | ClaudeScope `/scope` |
