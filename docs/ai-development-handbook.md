...
the field. It doesn't know a mechanism got bent in quarterfinals, that the new
battery sags, or that something makes a grinding noise above 3000 RPM.
...
- **Behavior** — what should happen, in order, with numbers.
  *"Spin the mechanism up, wait up to 1.5 s for it to reach setpoint, then feed."*
...
Vague specs produce vague code. **"Make the scoring mechanism better" is not a
spec.** "It overshoots by ~200 RPM when spinning up from idle; reduce overshoot
without slowing settling past 1.5 s" is a spec.
...
1. **Ask** — one clear sentence with numbers.
   *"Change the mechanism idle speed from 500 to 800 RPM — we measured spinup
...
5. **Hardware ground truth.** Which robot (practice vs competition), what's
   plugged in, what the mechanism looks like. A photo works — Claude reads images.
...
- **The stale session.** Hours across five topics — the vision debug bleeds
  into the mechanism tune. One task per session (`/clear`).
...
mechanism. The shape of the four lines is the part that transfers.)
...
outlives any one AI model — update the tools, keep the process. Carried into
the 2027 template from the 2026 season; last substantive update July 2026.*
